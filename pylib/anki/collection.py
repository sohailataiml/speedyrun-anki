...
class Collection(DeprecatedNamesMixin):
    ...
    def mastery_query(self, topics: list[str]) -> Sequence[stats_pb2.TopicMastery]:
        """Per-topic mastery (mean FSRS retrievability) and average recall,
        for topics represented as `topic::<name>` tags. Used by the
        three-score dashboard's Memory score and coverage map.
        """
        return self._backend.mastery_query(topics)

    def give_up_gate(self, topics: list[str]) -> stats_pb2.GiveUpGateResponse:
        """The give-up rule (PRD §5): whether there's enough data to score
        the given topics at all, checked *before* any score is computed.
        Check `response.WhichOneof("result")`: "data" means the Performance
        model may run; "insufficient" means the caller must show "no score"
        rather than compute one.
        """
        return self._backend.give_up_gate(topics)

    def performance_query(
        self, topics: list[str], average_difficulty: float, average_timing_seconds: float
    ) -> stats_pb2.PerformanceQueryResponse:
        """Predicts accuracy on held-back exam-style questions from topic
        mastery, difficulty, timing, and coverage (PRD §10.2). Runs the
        give-up gate first; check `response.WhichOneof("result")` the same
        way as `give_up_gate`.
        """
        return self._backend.performance_query(
            topics=topics,
